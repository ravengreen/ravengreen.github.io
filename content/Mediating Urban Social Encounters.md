---
tags:
  - project
---

> [!tip] Summary
> In this project, robotic street furniture was co-designed with 14 adolescents through theater-based methods to explore how robots could facilitate spontaneous social encounters in public space, resulting in 10 speculative concepts and 8 design suggestions that leverage robots' uniquely non-human qualities to support human-human connection.
## Context
### Initial Situation
Adolescents rely on urban public spaces for peer bonding and social exploration, but are increasingly digitally isolated and often excluded or regulated in physical public spaces. Spontaneous, serendipitous encounters, which support social resilience, are becoming harder to come by. Existing HCI work on socially facilitative technology typically treats systems as passive, responsive infrastructure rather than as agents with social initiative.
### Challenges
Robots in public spaces are mostly functional (cleaning, delivery, surveillance) and not designed to mediate human-human interaction. The little social robotics research that exists focuses on dyadic human-robot interaction, not on triadic or group dynamics. Adolescents specifically are almost entirely absent from HRI research, and their perspectives on robots as social enablers in public space are unexplored.
## Objective
As part of the research team at the University of Siegen with the Honda Research Institute EU I supported as a design researcher in facilitating a participatory, theater-based co-design process with adolescents. My role included the moderation of the focus group and parts of the co-design workshop, and three interviews.

**Research Questions**
- Could robots, precisely because of their physical presence, agency, and unique social qualities, improve human-human social interaction in public spaces?
- How might adolescents imagine robots that enable, disrupt, or reshape spontaneous encounters in public spaces?
- What are the unique social qualities of robots relevant to adolescents, and how can designers make use of them?
## Approach
### Procedure
![[Pasted image 20260518104802.png]]

### Methods and Tools
- Focus Group
- Interviews
- Co-Design Workshop with theater-based improvisation and role-play (inspired by Techno-Mimesis, Bodystorming, Speculative Enactments)
	- Participants embodied both human and robot roles using low-fidelity props/prostheses as robotic protheses (sleeping masks, roller boards, karaoke microphones, colored torches, etc.)
	- Robot profiles were completed after each enactment session for structured reflection
	- Full-group reflection phases to surface ethical concerns and method feedback
	- Collaboration with a participatory theater practitioner for warm-ups and facilitation support
### Analysis
- Reflexive thematic analysis of focus group/interview transcripts, videos, and concepts
## Result

### Outcome
- 8 design suggestions derived across 4 categories: facilitate contact and encounter; navigate social tensions and frictions; enable reflection and perspective-taking; mediate communication and exchange
- 10 speculative robot concepts co-created, including Roaming Benches, Talking Traffic Lights, Intervening Wall, and Complimenting Bollard
- Seven categories of robotic social roles identified: playful, therapeutic, rebellious, creative, dependent, connecting, and functional
### Insights
- Key robotic strengths: persistence, shamelessness, emotional constancy, impartiality, non-judgment, 24/7 availability
- Key robotic weaknesses: intrusiveness, tactlessness, naivety, potential for surveillance and discrimination
- The same trait can be strength or weakness depending on context — neutrality, for instance, can feel liberating or cold
- Adolescents preferred concepts with subtle, practical social affordances (Playful Bin, Assistive Bike Stands) over explicitly social ones — revealing a desire for plausible deniability when seeking connection
- Robots were most valued when they complemented human limitations rather than mimicking human behaviour

## Reflection
### Learnings
Being part as a researcher in this project, I gained deep insights into participatory, qualitative research. I learned how to translate qualitative data into design material, from interview themes to prompts to enacted concepts, and how embodied methods like role-play can surface insights that discussion alone cannot. Furthermore, I got a hands-on sense of what ethical, participatory research with a vulnerable group looks like in practice. 

## Links
Read the full paper [here](https://dl.acm.org/doi/epdf/10.1145/3772318.3791189)
